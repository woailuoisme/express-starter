...
const { pickRandom, randomDate } = require('../../common/random');
const { success } = require('../../common/logger');
...
async function reset() {
  ...
  for (const orchestra of await sequelize.models.orchestra.findAll()) {
    for (let i = 0; i < 10; i++) {
      const type = pickRandom(['violin', 'trombone', 'flute', 'harp', 'trumpet', 'piano', 'guitar', 'pipe organ']);
